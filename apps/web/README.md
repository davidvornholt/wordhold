...
`/courses/$courseId` carries course totals, the next useful action, the full vocabulary list, importing and settings (`src/features/courses/`). Its unit rows distinguish vocabulary that has not been introduced, first reviews, due reviews and the next scheduled date. `/courses/$courseId/units/$unitId` shows every vocabulary entry with the same per-direction schedule detail, then offers kennenlernen while entries remain unintroduced and free practice once the unit has introduced entries. An empty unit offers neither action.

`/courses/$courseId/vocabulary` is the searchable schedule view for the whole course. It filters by unit, due cards, first reviews and difficult vocabulary. Every entry exposes the next date, failures and the three most recent results for both directions. Checkboxes can combine vocabulary across units into one free practice session. The difficult-vocabulary view can select all of its introduced entries with one action.
...
The feedback panel names the result and immediately shows the resulting schedule. A missed card no longer returns after an arbitrary number of other cards. It enters FSRS relearning. At the end of the current 20-card section it appears in a named after-round only if its FSRS time has arrived. Otherwise the summary shows its next date. This avoids immediate answer recall without keeping a session open until a future timer expires. The progress bar counts distinct cards and never moves backwards. An ungraded answer leaves both learning state and date unchanged.

Large free-practice selections are divided into 20-card sections. Each checkpoint offers another section or a clean stop. The summary separates first-try answers, after-round recoveries, cards still uncertain and answers the judge could not grade. It also shows the earliest next review and any remaining scheduled backlog.

## Free practice

Reviews should mix across a course, but a learner may also prepare one unit or a hand-picked set. `/courses/$courseId/study` accepts either a unit or selected vocabulary from any number of units. It includes introduced cards whether due or not and offers either direction even when that direction is disabled for the regular plan.
